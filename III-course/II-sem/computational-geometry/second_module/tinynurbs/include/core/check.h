...
bool curveIsValid(unsigned int degree, const std::vector<T> &knots,
  ...
  return isKnotVectorMonotonic(knots);
}
...
bool curveIsValid(unsigned int degree, const std::vector<T> &knots,
  ...
  return isValidRelation(degree, knots.size(), control_points.size()) && weights.size() == control_points.size();
}
...
bool surfaceIsValid(unsigned int degree_u, unsigned int degree_v,
  ...
  return !(!isValidRelation(degree_u, knots_u.size(), control_points.rows()) ||
...
bool surfaceIsValid(unsigned int degree_u, unsigned int degree_v,
  ...
  return surfaceIsValid(degree_u, degree_v, knots_u, knots_v, control_points)
      && !(control_points.rows() != weights.rows() ||
